...
export class DraggableGridComponent implements OnInit, OnChanges {
  ...
  @Output() positionsChanged: EventEmitter<DragGridPosition[] | null> =
    new EventEmitter();
  ...
  private readItems(): DragGridItem[] | null {
    ...
    if (
      this.dataGridSource &&
      this.dataGridSource.cols &&
      this.dataGridSource.cols.length > 0 &&
      endPosition
    ) {
  ...
  private getDragGridItem(position: GridPosition): DragGridItem | null {
    ...
    if (
      this.dataGridSource &&
      this.dataGridSource.cols &&
      colIndex < this.dataGridSource.cols.length
    ) {
  ...
  private getValidEmptyEndPosition(): {
    colIndex: number;
    rowIndex: number;
  } | null {
    if (
      ...
      const initialRowsSize = Math.floor(
        this.dataSource.length / this.dataGridSource.cols.length
      );
